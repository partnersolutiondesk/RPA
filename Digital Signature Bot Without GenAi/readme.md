...
This bot determines whether a digital signature is present in a document processed through **Automation Anywhere Document Extraction** without using any Generative AI capabilities.

The solution analyzes the JSON output generated by Document Extraction and checks whether a **SIGNATURE** block exists within the OCR-detected feature objects.
...
How can we verify whether a digital signature is present in a document?

## Proposed Solution

Use **Automation Anywhere Document Extraction** to identify signature blocks from the OCR output and then determine whether a signature exists using:
...
# Signature Detection Using JSON Utils

## Step 1: Configure Document Extraction Download Action
...
Start a JSON session using the downloaded JSON file path.
...
# Signature Detection Using Python

The logic remains the same. Only the implementation method changes.

---

## Step 1: Combine Multiple Inputs into a Single Parameter

First, download the Document Extraction output as JSON and use this file as an input.

Next, pass the document JSON that needs to be updated with the signature detection result.

![alt text](screenshots/image-6.png)

The Python Action in Automation Anywhere accepts only one function and one input parameter. In this use case, two separate values must be passed:

1. The file path of the Document Extraction JSON output.
2. The documentData JSON that needs to be updated.

To achieve this, create a single JSON string that contains both values.

```json
{
  "filePath": "$SampleString$",
  "documentData": "$recordIN{documentJSON}$"
}
```

![alt text](screenshots/Screenshot%202026-07-23%20145204.png)

---

## Step 2: Call the Python Action

Send the combined JSON input to the Python function.

![alt text](screenshots/image-4.png)

![alt text](screenshots/Screenshot%202026-07-23%20145726.png)

---

## Step 3: Update the Document

Use the output returned by the Python function to update the document object.

![alt text](screenshots/Screenshot%202026-07-23%20150244.png)
...
```text
signatureDetection.py
```

The Python script performs the following:

1. Reads the Document Extraction JSON output.
2. Retrieves the featureObjects collection.
3. Searches for a block whose blockType is SIGNATURE.
4. Sets the signature detection status.
5. Updates and returns the documentData JSON.
